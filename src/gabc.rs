...
impl Gabc {
    ...
    pub fn split_verses(&self) -> Result<Vec<Self>> {
        // Split score by line
        let mut verses = self
            .score
            .split('\n')
        ...
        let mut verses_w_clef: Vec<String> = Vec::new();
        verses_w_clef.push(verses[0].to_owned());
        for v in &verses[1..] {
            let v_with_clef = clef.to_string() + v;
            verses_w_clef.push(v_with_clef);
        }

        let mut gabc_verses: Vec<Self> = Vec::new();
        for v in verses_w_clef {
            let new_gabc = Gabc {
                header: self.header.clone(),
                score: v,
            };
            gabc_verses.push(new_gabc);
        }
